...
sleep(2000);
/* 不在首页的跳转到首页 */
while (!selector().className('android.widget.TextView').text('输入市内、城际目的地').findOne(1500))
    back();
...
for (let i = 0; i < 3; i++) {
    let temp = selector()
        ...
        .depth(0)
        .findOne(500);
    if ((temp === null || temp === void 0 ? void 0 : temp.childCount()) === 2) {
        temp.child(1).click();
        break;
    }
    else {
...
(_a = selector()
    .className('android.widget.TextView')
    .text('立即签到')
    .findOne(2000)) === null || _a === void 0 ? void 0 : _a.click();
/* 执行完毕退出程序返回到最开始的桌面 */
back();
sleep(250);
back();
...
const didaTitleRect = selector()
    ...
    .findOne()
    .bounds();
swipe(didaTitleRect.centerX(), didaTitleRect.centerY(), didaTitleRect.centerX() + 2080, didaTitleRect.centerY(), 300);
/* 锁定设备 */
utils.lockDevice();
